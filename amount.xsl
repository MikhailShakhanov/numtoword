...
<xsl:template name="num-to-word">
 ...
 <xsl:with-param name="value" select="floor(number($value2))"/>
 ...
 <xsl:choose>
   <xsl:when test="floor(number($value2))!=$value2">
     <xsl:variable name="kop" select="round((number($value2)-floor(number($value2)))*100)"/>
      <xsl:call-template name="float2speech">
        <xsl:with-param name="value" select="$kop"/>
        <xsl:with-param name="sex" select="m"/>
        <xsl:with-param name="power" select="0"/>
      </xsl:call-template>
   </xsl:when>
   <xsl:otherwise>
     <xsl:value-of select="'ноль копеек'"/>
   </xsl:otherwise>
 </xsl:choose>
 ...
 <xsl:variable name="i" select="floor(number($value))"/>
 ...
 <xsl:value-of select="concat($ret,$ret2,$ret3)"/>
  ...
  <xsl:template match="/">
    <xsl:call-template name="num-to-word">
      <xsl:with-param name="value" select="translate/amount/node()"/>
    </xsl:call-template>
 </xsl:template>
